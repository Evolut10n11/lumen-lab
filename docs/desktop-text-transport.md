...
This transport fix deliberately does not reset or heuristically rewrite existing profiles. Text already persisted through a wrong decoder may require a separate, user-confirmed repair; characters previously discarded cannot be reliably guessed. Do not delete AppData as an encoding workaround.
