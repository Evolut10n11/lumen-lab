...
- Short one-character Cyrillic words such as `Я` are restored even when they touch punctuation inside otherwise-correct localized mission text. Ambiguous valid text such as `Рё` is left unchanged. A v0.2.1 backup enables this only when it exactly proves that the live file is the corresponding incompletely repaired state.
